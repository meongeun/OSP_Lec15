...
from path import voc_dir, model_dir
...
PATH = model_dir + 'resnet_encoder_unet.pth'

checkpoint = torch.load(PATH, map_location=device)
#tmp = {key: value for key, value in checkpoint.items()}
#print('\n'.join(["{} : {}".format(key, tmp[key].shape) for key in sorted(tmp.keys())]))
...
result_save_dir = './history/resnet_result'+date+'/'
...
print("Initial loss : {}, accuracy : {}".format(train_loss, train_acc))
...
for epoch in range(epochs):
    ...
    if epoch % 4 == 0:
        ...
        torch.save(model.state_dict(), PATH[:-4]+'_{}.pth'.format(epoch))
